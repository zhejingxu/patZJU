...
long long num2Dec(char * p,long long radix)  //将输入的字符串转化成long long类型的长整数
{  
...
long long findLowRadix(char *p)  //对于输入的字符串需找其最小的radix，即检查每一位都小于radix
{  
    long long len=strlen(p);  
    long long low=0;  
    long long num;  
...
int main(void){	
    ...
    {  
        ...
        most = (target + 1 > least + 1) ? target +1 :least +1;   //最大可能的radix
