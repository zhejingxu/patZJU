...
int main()
{
	...
	//注意对数组进行sort运算，第一个参数是数组的第一个元素，第二个参数是最后一个元素的再后一个元素
	...
	//players的处理游标，标记第一个未被处理的player
    int cur = 1;
    int timer = 0;

	//while循环每次处理一个pair和一个桌子（因为pair是先后顺序的，所以每次可以只处理一个）
    while(timer < END) {

		//将到当前时间timer前到的所有pair都放入到deque中
        while (cur <= n && players[cur].arrive <= timer){
			waiting.push_back(cur ++);
		}
        
		//如果当前时间没有pair到达（等待队列为空），则选择最新的一个pair，用其arrive time更新timer
        if (waiting.size() == 0) {
            ...
            if (cur <= n && players[cur].arrive < END) {
				...
				//更新了timer之后，将原先就available的桌子的可用时间更新为timer
                for (int i = 1; i <= k; ++ i) {  
        ...
        int stt = timer;
		//floor(x)取不大于x的最大整数，若floor(9.99) = 9 本题要满足3.5->4 
